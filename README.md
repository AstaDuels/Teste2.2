...
-- GLOBALS
-- ============================================================
local NS = 60  -- Velocidade normal (quando o speed personalizado está desligado)
local LAGGER_SPEED = 15
local LAGGER_CARRY_SPEED = 24.5  -- Mantido para lagger carry (não relacionado ao speed)
...
-- ============================================================
-- NOVO SISTEMA DE VELOCIDADE PERSONALIZADA (estilo v4gg.xyz)
-- ============================================================
local customSpeedEnabled = false
local customSpeedValue = 60  -- valor padrão
...
local function applyCustomSpeed(spd)
    if customSpeedConn then
        customSpeedConn:Disconnect()
        customSpeedConn = nil
    end
    ...
    customSpeedConn = RunService.Heartbeat:Connect(function(dt)
        if not customSpeedEnabled then return end
...
local function toggleCustomSpeed()
    ...
    if customSpeedEnabled then
        ...
        if hum then hum.WalkSpeed = NS end
        lastPosition = nil
        lastTime = nil
    end
    if mobBtnRefs.speed then mobBtnRefs.speed(customSpeedEnabled) end
    saveConfig()
end
...
local autoSwitchSpeedEnabled = false  -- Removido, não será usado
...
local KB = {
    ...
    SpeedToggle   = {kb = Enum.KeyCode.Q,           gp = nil},  -- Novo: alternar speed personalizado
...
-- ============================================================
-- MOVEMENT & SPEED (adaptado para novo sistema)
-- ============================================================
...
local function getActiveMoveSpeed()
    ...
    elseif customSpeedEnabled then return customSpeedValue
    else return NS end
end

local function getAutoPathSpeed()
    if laggerCarryToggled or laggerToggled then return LAGGER_SPEED else return NS end
end
...
-- Removidas funções toggleCarryMode, toggleLaggerMode, toggleLaggerCarryMode mantidas, mas sem speedMode
function toggleLaggerMode()
    if laggerCarryToggled then laggerCarryToggled = false end
    laggerToggled = not laggerToggled
    laggerPhase = laggerToggled and 1 or 0
    refreshSpeedModeLabel()
end

function toggleLaggerCarryMode()
    if laggerToggled then laggerToggled = false; laggerPhase = 0 end
...
-- ============================================================
-- AUTO SWITCH SPEED (removido, não faz mais sentido)
-- ============================================================
...
-- ============================================================
-- STEAL SYSTEM (with progress bar updates) – unchanged
-- ============================================================
local Steal = {
...
local function stopAutoSteal()
    ...
    isStealing = false; lastStealTick = 0; Steal.cachedPrompts = {}; resetProgressBar()
end
...
local function stopAutoLeft()
    ...
    alPhase = 1; autoLeftEnabled = false
...
local function stopAutoRight()
    ...
    arPhase = 1; autoRightEnabled = false
    ...
    alConn = RunService.Heartbeat:Connect(function()
        ...
        if alPhase == 1 then
            ...
            if (tgt - hrp.Position).Magnitude < 1 then
                ...
                hum:Move(mv, false); hrp.AssemblyLinearVelocity = Vector3.new(mv.X*spd, hrp.AssemblyLinearVelocity.Y, mv.Z*spd); hum:Move(mv, true); return
            end
            local d = AP_L1 - hrp.Position; local mv = Vector3.new(d.X,0,d.Z).Unit
            hum:Move(mv, false); hrp.AssemblyLinearVelocity = Vector3.new(mv.X*spd, hrp.AssemblyLinearVelocity.Y, mv.Z*spd); hum:Move(mv, true)
...
local function onAnchorChanged(part)
    return part:GetPropertyChangedSignal("Anchored"):Connect(function()
        if part.Anchored and part.Transparency == 1 then
            if medusaResetEnabled then cursedInstaReset()
...
function saveConfig()
    ...
    if stealBarFrame then
        stealPos = {x = stealBarFrame.Position.X.Offset, y = stealBarFrame.Position.Y.Offset}
    end
    local cfg = {
        normalSpeed = NS,
        customSpeedEnabled = customSpeedEnabled,
        ...
        laggerMode = laggerToggled, laggerCarryMode = laggerCarryToggled,
        ...
        antiKickEnabled = antiKickEnabled, infJumpMode = infJumpMode,
        ...
        stealBarPos = stealPos,
    }
    pcall(function() writefile("AstaPC.json", HS:JSONEncode(cfg)) end)
end

task.spawn(function() while task.wait(5) do saveConfig() end end)

-- ============================================================
-- HEAD LABELS (unchanged)
-- ============================================================
local function createHeadLabels(char)
    ...
    local old2 = char:FindFirstChild("VynxDiscordLabel")
    if old2 then old2:Destroy() end

    local discordGui = Instance.new("BillboardGui")
    discordGui.Name = "VynxDiscordLabel"
    discordGui.Adornee = head
    discordGui.Size = UDim2.new(0, 220, 0, 20)
    discordGui.StudsOffset = Vector3.new(0, 4.2, 0)
    discordGui.AlwaysOnTop = true
    discordGui.Parent = char
...
-- ============================================================
-- ADD SPEED LABELS FOR OTHER PLAYERS (unchanged)
-- ============================================================
...
Players.PlayerRemoving:Connect(function(plr)
    if playerSpeedLabels[plr] then
        playerSpeedLabels[plr].gui:Destroy()
        playerSpeedLabels[plr].conn:Disconnect()
        playerSpeedLabels[plr] = nil
    end
end)
...
LP.CharacterAdded:Connect(function(char)
    _cachedChar = nil; _cachedHum = nil; _cachedHrp = nil; Steal.Data = {}
    task.wait(0.3)
    _cachedChar = char
    ...
    if Steal.AutoStealEnabled then stopAutoSteal(); startAutoSteal() end
    -- Reaplicar speed personalizado se estiver ativo
    if customSpeedEnabled then
        task.wait(0.2)
        applyCustomSpeed(customSpeedValue)
    end
...
-- ============================================================
-- RENDER STEPPED (optimised – skip if not needed)
-- ============================================================
...
RunService.RenderStepped:Connect(function()
    local char = LP.Character
    if not char then _cachedChar = nil; return end
    local hum, hrp
    if char == _cachedChar then hum = _cachedHum; hrp = _cachedHrp
    else
        _cachedChar = char; hum = char:FindFirstChildOfClass("Humanoid")
        hrp = char:FindFirstChild("HumanoidRootPart"); _cachedHum = hum; _cachedHrp = hrp
    end
    if not hum or not hrp then return end
    local _st = hum:GetState()
    if _st == Enum.HumanoidStateType.Physics or _st == Enum.HumanoidStateType.Ragdoll or _st == Enum.HumanoidStateType.FallingDown then
        lastMoveDir = Vector3.new(0,0,0); return
    end
    -- Apenas aplica movimento se não houver nenhum dos sistemas ativos que controlam velocidade (TP Lock, Bat V2, Auto Left/Right)
    if not tpLockEnabled and not autoLeftEnabled and not autoRightEnabled and not batV2Enabled and not customSpeedEnabled then
...
UIS.InputBegan:Connect(function(input, gpe)
    ...
    elseif kbMatch(KB.SpeedToggle, kc) then toggleCustomSpeed(); saveConfig()
...
-- ============================================================
-- MOBILE BUTTONS (atualizado)
-- ============================================================
local MOB_POS_FILE = "asta_btnpos.json"
...
local function resetBtnPositions()
    ...
    if stealBarFrame then
        stealBarFrame.Position = UDim2.new(0.5, -150, 0.85, 0)
        saveConfig()
    end
end
...
local function buildMobileButtons()
    ...
    local buttonOrder = {
        ...
        {key="tpLock",       label="TP LOCK",         toggle=true},
        {key="lagger",       label="LAGGER",          toggle=true},
        {key="laggerCarry",  label="LAGGER CARRY",    toggle=true},
        {key="speed",        label="SPEED",           toggle=true},  -- Antigo CARRY SPD
    ...
    for i, def in ipairs(buttonOrder) do
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if def.key == "speed" then
                toggleCustomSpeed(); setOn(customSpeedEnabled)
                if mobBtnRefs.lagger then mobBtnRefs.lagger(laggerToggled) end
                if mobBtnRefs.laggerCarry then mobBtnRefs.laggerCarry(laggerCarryToggled) end
                saveConfig(); return
            end
            ...
            if def.key == "taunt" then
                local message = "vynx is better ur trash lol"
...
local function createStealBar()
    for _, n in ipairs({"VynxStealBar"}) do
...
local function loadConfig()
    if not (isfile and isfile("AstaPC.json")) then return end
    local ok, cfg = pcall(function() return HS:JSONDecode(readfile("AstaPC.json")) end)
    if not ok or type(cfg) ~= "table" then return end
    if cfg.normalSpeed then NS = cfg.normalSpeed end
    if cfg.customSpeedEnabled ~= nil then customSpeedEnabled = cfg.customSpeedEnabled end
    if cfg.customSpeedValue then customSpeedValue = cfg.customSpeedValue end
    ...
    if cfg.antiKickEnabled ~= nil then antiKickEnabled = cfg.antiKickEnabled end
    ...
    applyKey(KB.DropBrainrot, cfg.dropBrainrotKey); applyKey(KB.AutoLeft, cfg.autoLeftKey)
    applyKey(KB.AutoRight, cfg.autoRightKey); applyKey(KB.TPLock, cfg.tpLockKey)
...
local function applyState()
    ...
    if infJumpEnabled then if infJumpMode == "hold" then startHoldInfJump() end end
    ...
    if antiKickEnabled then enableAntiKick() end
    ...
    -- Aplicar speed personalizado se estiver ativo
    if customSpeedEnabled then
        task.wait(0.3)
        applyCustomSpeed(customSpeedValue)
    end
...
-- ============================================================
if _G.ASTA_LOADED then return end
_G.ASTA_LOADED = true

-- ============================================================
-- BUILD ASTA HUB GUI (draggable from anywhere)
-- ============================================================
local function buildGui()
    ...
    local DIM_LINE = Color3.fromRGB(28,  28,  34)
    local CHECK_OFF= Color3.fromRGB(30,  30,  30)
    local BTN_ACT  = Color3.fromRGB(38,  40,  48)
    ...
    bgImage.Image = "rbxassetid://85719239686756"  -- Novo fundo
    ...
    Title.BackgroundTransparency = 1; Title.Text = "ASTA HUB"
    ...
    -- TAB SYSTEM – removida a aba "Songs"
    local currentTab = nil
    local TabRefs = {contents = {}, btns = {}, underlines = {}, active = "Speed"}
    local TabList = {"Speed","Combat","Mechns","Visual","Extra"}
    ...
    -- ===== NOVA ABA SPEED =====
    mkSect(sfSpeed, "Speed Control")

    -- Campo para digitar velocidade
    local speedRow = mkRow(sfSpeed)
    mkLabel(speedRow, "Custom Speed")
    ...
    speedInput.BackgroundColor3 = INP_BG; speedInput.Text = tostring(customSpeedValue)
    ...
    speedInput.FocusLost:Connect(function()
        ...
        if v and v > 0 and v <= 500 then
            ...
            speedInput.Text = tostring(customSpeedValue)
        end
    end)
    ...
    -- Botão Speed (toggle)
    local speedBtnRow = mkRow(sfSpeed)
    mkLabel(speedBtnRow, "Enable Speed")
    local speedToggleBtn = Instance.new("TextButton", speedBtnRow)
    speedToggleBtn.Size = UDim2.new(0,80,0,26); speedToggleBtn.Position = UDim2.new(1,-85,0.5,-13)
    speedToggleBtn.BackgroundColor3 = customSpeedEnabled and Color3.fromRGB(0,200,80) or BTN_ACT
    speedToggleBtn.BorderSizePixel = 0
    speedToggleBtn.Text = customSpeedEnabled and "ON" or "OFF"
    speedToggleBtn.TextColor3 = WHITE
    ...
    local speedToggleStroke = Instance.new("UIStroke", speedToggleBtn)
    speedToggleStroke.Color = WHITE; speedToggleStroke.Thickness = 1
    ...
    -- Chave de atalho (SpeedToggle)
    mkActionRow(sfSpeed, "Keybind (Speed)", nil, KB.SpeedToggle)

    -- Normal Speed (valor padrão)
    ...
    -- ===== COMBAT TAB (inalterado) =====
    mkSect(sfCombat, "Keybinds")
    mkActionRow(sfCombat, "Speed Toggle",  nil, KB.SpeedToggle)  -- já incluso acima, mas mantido para consistência
    ...
    -- NO INTRO – mostra GUI instantaneamente
    mainFrame.Visible = true
    miniToggleBtn.Visible = false
    print("ASTA HUB GUI Fully Loaded")
end

-- ============================================================
-- INIT (asynchronous) – optimised startup
-- ============================================================
loadConfig()
uiScaleValue = 0.6
buildGui()
...
print("ASTA HUB Fully Loaded – Bat V2 uses your specified settings: Chase 58, Hit 8, Cooldown 0.35, Height 3.7, Y -70..110")
